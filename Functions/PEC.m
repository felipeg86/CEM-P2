classdef PEC
    ...
    function v = potencial(obj,obsPoint)
            ...
            v(:,:,1) = P0.*log((norm(R_p(:,:,1))+norm(l_p(:,:,1)))./(norm(R_m(:,:,1))+norm(l_m(:,:,1)))) - ...
                abs(d).*(atan((P0.*norm(l_p(:,:,1)))./((R0.^2+abs(d).*norm(R_p(:,:,1)))))-...
                atan((P0.*l_m(:,:,1))./((R0.^2+abs(d).*norm(R_m(:,:,1))))));

            v(:,:,2) = P0.*log((norm(R_p(:,:,2))+norm(l_p(:,:,2)))./(norm(R_m(:,:,2))+norm(l_m(:,:,2)))) - ...
                abs(d).*(atan((P0.*norm(l_p(:,:,2)))./((R0.^2+abs(d).*norm(R_p(:,:,2)))))-...
                atan((P0.*l_m(:,:,2))./((R0.^2+abs(d).*norm(R_m(:,:,2))))));
            
            v(:,:,3) = P0.*log((norm(R_p(:,:,3))+norm(l_p(:,:,3)))./(norm(R_m(:,:,3))+norm(l_m(:,:,3)))) - ...
                abs(d).*(atan((P0.*norm(l_p(:,:,3)))./((R0.^2+abs(d).*norm(R_p(:,:,3)))))-...
                atan((P0.*norm(l_m(:,:,3)))./((R0.^2+abs(d).*norm(R_m(:,:,3))))));
